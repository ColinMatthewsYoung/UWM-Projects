...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  uint16_t pwm = processADC(&hadc1, &hlpuart1);

	  //adc flex returns a value around 2300 when there is no flex. The two extreams are anything under 2250, and anything over 2500.
	  if(pwm < 2250){
...
void HAL_TIM_PeriodElaspedCallback(TIM_HandleTypeDef *htim){
	...
	else if(timer_ar_value == TIM1_PWM_FREQ_4K){
		timer_ar_value = TIM1_PWM_FREQ_4K;
		__HAL_TIM_SET_AUTORELOAD(&htim1, TIM1_PWM_FREQ_4K);
		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, TIM1_PWM_4K_25DUTY);
		__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, TIM1_PWM_4K_50DUTY);
